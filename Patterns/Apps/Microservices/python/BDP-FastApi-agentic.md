...
| **API Main**  | `api/main.py`    | Main entry point for the FastAPI application, including app initialization, middleware setup, and route inclusion from submodules. | `main.py`        | FastAPI App Instance | Includes routers from `api/` submodules, starts the server.                                |
